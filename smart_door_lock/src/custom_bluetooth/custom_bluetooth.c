#include <zephyr/bluetooth/bluetooth.h>
#include <zephyr/bluetooth/conn.h>
#include <zephyr/bluetooth/gatt.h>
#include <zephyr/mgmt/mcumgr/transport/smp_bt.h>
#include <zephyr/sys/printk.h>

static struct k_work advertise_work;

static const struct bt_data ad[] = {
    BT_DATA_BYTES(BT_DATA_FLAGS, (BT_LE_AD_GENERAL | BT_LE_AD_NO_BREDR)),
    BT_DATA_BYTES(BT_DATA_UUID128_ALL, 0x84, 0xaa, 0x60, 0x74, 0x52, 0x8a, 0x8b, 0x86, 0xd3, 0x4c, 0xb7, 0x1d, 0x1d,
                  0xdc, 0x53, 0x8d),
};

static void advertise(struct k_work *work)
{
    int rc;

    bt_le_adv_stop();

    rc = bt_le_adv_start(BT_LE_ADV_CONN_NAME, ad, ARRAY_SIZE(ad), NULL, 0);
    if (rc)
    {
        printk("Advertising failed to start (rc %d)", rc);
        return;
    }

    printk("Advertising successfully started");
}

static void connected(struct bt_conn *conn, uint8_t err)
{
    if (err)
    {
        printk("Connection failed (err 0x%02x)", err);
    }
    else
    {
        printk("Connected via BLE");
    }
}

static void disconnected(struct bt_conn *conn, uint8_t reason)
{
    printk("Disconnected via BLE(reason 0x%02x)", reason);
    k_work_submit(&advertise_work);
}

BT_CONN_CB_DEFINE(conn_callbacks) = {
    .connected = connected,
    .disconnected = disconnected,
};

static void bt_ready(int err)
{
    if (err != 0)
    {
        printk("Bluetooth failed to initialise: %d", err);
    }
    else
    {
        k_work_submit(&advertise_work);
    }
}

void start_smp_bluetooth_adverts()
{
    int rc = -1;

    if (!k_work_is_pending(&advertise_work))
    {
        k_work_init(&advertise_work, advertise);
    }

    if (!bt_is_ready())
    {
        rc = bt_enable(bt_ready);
    }

    if (rc != 0)
    {
        printk("Bluetooth enable failed: %d", rc);
    }
}

void stop_smp_bluetooth_adverts()
{
    if (k_work_is_pending(&advertise_work))
    {
        k_work_cancel(&advertise_work);
    }

    if (bt_is_ready())
    {
        bt_disable();
    }
}